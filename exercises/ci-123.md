...
Push your local code up to the remote repository, which now should also trigger the execution of automated tests on the CI server:

```sh
git push origin master
```

Visit your repository's CI "builds" at a URL like `https://travis-ci.com/GITHUB_USERNAME/GITHUB_REPONAME` to see the results of the automated tests and verify Travis has been configured properly:
...
Edit the code in the application file:

```py
```

Save and commit these changes, then push them to the "my-new-feature" branch:

```sh
```

You should be able to view your branch online, and open a Pull Request to propose it be merged into the "master" branch:

```sh
```

Opening a PR should trigger a CI build, which in this case we expect to be failing.

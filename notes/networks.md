...
This course focuses primarily on `GET` requests. The most common way a computer can issue a `GET` request is by visiting some URL in a browser. But a computer is also capable of programmatically issuing `GET` requests using command-line utilities and modern programming languages. Like other programming languages, [Python provides a way](/notes/python/packages/requests.md) for the developer to write programs which issue HTTP requests.
